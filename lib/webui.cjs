const DEFAULT_OS_TEMPLATE = 'Debian (latest)';
...
async function normalizeDynamicValues(page) {
  await page.evaluate(() => {
    ...
    const trafficRowValues = new Map([
      ['Přijato', ['1,22 KiB', '19']],
      ['Odesláno', ['290,57 KiB', '3,05k']],
      ['Celkem', ['291,79 KiB', '3,07k']],
      ['Received', ['1.22 KiB', '19']],
      ['Sent', ['290.57 KiB', '3.05k']],
      ['Total', ['291.79 KiB', '3.07k']],
    ]);
    for (const row of document.querySelectorAll('tr')) {
      if (row.querySelector('input, select, textarea, button')) continue;
      const cells = Array.from(row.cells);
      const label = cells[0]?.textContent.trim().replace(/:$/, '');
      const tableText = row.closest('table')?.textContent || '';
      const values = trafficRowValues.get(label)
        || (/Uptime/.test(tableText) ? statusRowValues.get(label) : undefined);
      if (!values) continue;
...
async function walkCreateVps(
  page,
  {
    ...
    locationLabel = 'Praha',
    templateLabel = DEFAULT_OS_TEMPLATE,
  } = {},
) {
  ...
  await selectRadioByRowText(form, 'os_template', templateLabel);
  await submitLast(form);
  await page.waitForLoadState('domcontentloaded');
  await preparePage(page);
